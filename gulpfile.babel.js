...
const paths = {
  ...
  js: {
    ...
    bundles: ['js/main.js', 'js/restaurant_info.js', 'serviceworker.js'],
  },
};
...
Object.keys(paths).forEach(prop => {
  ...
  log(copy.src[copy.src.length - 1]);
});
...
gulp.task('build', function(done) {
  return runSequence(
    'clean',
    ['responsive:images', 'js:bundle', 'sass'],
    'copy', // copy is done last, so is easy to see what's been copied.
    done
  );
});

//convert scss files to css in the dist folder
...
gulp.task('sync', ['build'], function() {
  ...
  gulp
    .watch(paths.responsive.src, ['responsive:images'])
...
function bundle(b, outputPath) {
  ...
  return (
    b
      ...
      .on('error', function(err) {
        log.error(c.red(`${outputFile} messed up ${err.toString()}`));
      })
      .on('end', function() {
  ...
  ); // call browserSync.stream() to refresh browser when using sync task
}
